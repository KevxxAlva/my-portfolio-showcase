...
export const useProjects = () => {
  ...
  const updateProject = async (id: string, updates: Partial<Project>) => {
    try {
      ...
      if (updates.descripcionCompleta !== undefined)
        dbUpdates.descripcion_completa = updates.descripcionCompleta;
      if (updates.tags !== undefined) dbUpdates.tags = updates.tags;
      if (updates.imagenUrl !== undefined)
        dbUpdates.imagen_url = updates.imagenUrl;
      if (updates.repoLink !== undefined)
        dbUpdates.repo_link = updates.repoLink;
      if (updates.demoLink !== undefined)
        dbUpdates.demo_link = updates.demoLink;
      if (updates.destacado !== undefined)
        dbUpdates.destacado = updates.destacado;
  ...
  const resetToDefault = () => {
    ...
    toast({
      title: "Info",
      description:
        "La restauración por defecto no está disponible en modo base de datos.",
    });
  };
